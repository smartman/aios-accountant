...
describe("saveAccountingConnection success cases", () => {
  ...
  it("uses the Merit adapter for Merit credentials", async () => {
    ...
    const formData = new FormData();
    formData.set("companyId", "merit-company");
...
describe("saveAccountingConnection provider changes", () => {
  it("updates the company provider before saving different provider credentials", async () => {
    const [
      { saveAccountingConnection },
      { getUser },
      { upsertAccountingConnection },
      ...
      import("@/lib/user-accounting-connections"),
      import("@/lib/merit"),
      import("@/lib/companies/repository"),
    ]);
    ...
    expect(updateCompanyAccountingProvider).toHaveBeenCalledWith({
      companyId: "company-1",
      user: {
        id: "vitest-dashboard-user",
        email: "vitest@example.com",
    ...
    expect(upsertAccountingConnection).toHaveBeenCalledOnce();
    expect(result.provider).toBe("merit");
  });
});
