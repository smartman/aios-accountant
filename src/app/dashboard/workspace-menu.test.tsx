...
describe("workspace-menu", () => {
  it("builds import and settings actions for connected workspaces", () => {
  ...
  it("builds visible section and status labels", () => {
    expect(getWorkspaceSectionLabel("settings")).toBe("Company settings");
    expect(getWorkspaceSectionLabel("import")).toBe("Import invoices");
    expect(getWorkspaceStatusLabel(false, null, "Acme")).toBe(
      "Acme: verify credentials before importing invoices.",
    );
    expect(getWorkspaceStatusLabel(true, "merit", "Acme")).toBe(
      "Acme: Merit is connected.",
    );
  });
  ...
  it("renders both actions in the visible main menu", () => {
    ...
    expect(markup).toContain("Company settings");
  ...
  it("fires enabled actions and ignores disabled ones", () => {
    ...
    hostProps(findButton(enabledTree, "Company settings")!).onClick?.();
    expect(actionIds).toEqual(["settings"]);
